...
// The tree
  ...
  float pi_eta[3];
  float pi_phi[3];
  TTree *tree = new TTree("triplet","Pion triplet");
  tree->Branch("pi1_pt", &pi_pt[0], "pi1_pt/F");
  tree->Branch("pi2_pt", &pi_pt[1], "pi2_pt/F");
  tree->Branch("pi3_pt", &pi_pt[2], "pi3_pt/F");
  tree->Branch("pi1_eta", &pi_eta[0], "pi1_eta/F");
  tree->Branch("pi2_eta", &pi_eta[1], "pi2_eta/F");
  tree->Branch("pi3_eta", &pi_eta[2], "pi3_eta/F");
  tree->Branch("pi1_phi", &pi_phi[0], "pi1_phi/F");
  tree->Branch("pi2_phi", &pi_phi[1], "pi2_phi/F");
  tree->Branch("pi3_phi", &pi_phi[2], "pi3_phi/F");
   ...
   // start of the loop
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      */
      ...
      if(JpsiTau_tau_pi3_pt->size() < JpsiTau_tau_pi2_pt->size())
      {
        std::cout << "2prongs exist" << std::endl;
      }
      if(JpsiTau_tau_pi2_pt->size() < JpsiTau_tau_pi1_pt->size())
      {
        std::cout << "1prongs exist" << std::endl;
      }
      ...
      for(int i=0; i < JpsiTau_tau_pi2_pt->size(); i++){
        h1pt2->Fill(JpsiTau_tau_pi2_pt->at(i));
      }
      for(int i=0; i < JpsiTau_tau_pi3_pt->size(); i++){
